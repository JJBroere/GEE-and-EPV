...
BetaTrue <- c(0.8,0.8,-0.5,-0.5,-0.3,-0.3,0,0,0,0)
for(i in 1:100){
  MESCAD[i] <- t(EstimatesSCAD10010u[,i]-BetaTrue)%*%(EstimatesSCAD10010u[,i]-BetaTrue)
}
...
UnderselectPGEE <- mean(EstimatesSCAD10010u[1:6,]==0)
#Overselection
OverselectBACK <- 1-mean(EstimatesBack[7:10,]==0)
OverselectPGEE <- 1-mean(EstimatesSCAD10010u[7:10,]==0)
UnderselectBACK
UnderselectPGEE
OverselectBACK
OverselectPGEE
...
EstimatesPGEEWithout <- EstimatesSCAD10010u
...
for(j in 1:100){
  ...
  GEEX1coef<- summary(geeglm(outcome ~ V2+V3+V4+V5+V6+V7+V8+V9+V10+V11, id= id, data=Sim100N100P10u[[j]], corstr="exchangeable", family = binomial))
