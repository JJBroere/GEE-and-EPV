...
BetaTrue <- c(0.8,-0.5,-0.3,0,0)
for(i in 1:100){
  MESCAD[i] <- t(EstimatesSCAD4005[,i]-BetaTrue)%*%(EstimatesSCAD4005[,i]-BetaTrue)
}
...
UnderselectPGEE <- mean(EstimatesSCAD4005[1:3,]==0)

#Overselection
OverselectBACK <- 1-mean(EstimatesBack[4:5,]==0)
OverselectPGEE <- 1-mean(EstimatesSCAD4005[4:5,]==0)
UnderselectBACK
UnderselectPGEE
OverselectBACK
OverselectPGEE
...
EstimatesPGEEWithout <- EstimatesSCAD4005
...
for(j in 1:100){
  ...
  GEEX1coef<- summary(geeglm(outcome ~ V2+V3+V4+V5+V6, id= id, data=Sim100N40P5u[[j]], corstr="exchangeable", family = binomial))
