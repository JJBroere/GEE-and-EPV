...
source("Estimators and CV 4 4010u.R")
load('Sim100N40P20uu.Rda')
######
###Tuning parameter selection  by (leaf one subject out) cross-validation: example SCAD parameter
######
...
library(matlab)
    ...
    M_diff <- 0.0000001 #(convergence indicator= precision)
    method <- 'SCAD'
  ...
  EstimatesSCAD4020u <- matrix(1,20,nsim)
Optimal4020uu1 <- matrix(1,50,2)
...
for(jj in 1:100){
    for (i_grid in 1:n_grid){
      ...
      resultaat <-  CV.PEE_LI(data=Sim100N40P20uuu[[jj]])
    ...
    Optimal4020uu[jj,] <- as.numeric(grid[LOW_MSE[1],])
    }
save(Optimal4020uu, file="Optimal4020uu.Rda")


  for(jj in 1:nsim){
    alpha <- Optimal4020uu[jj,2] #parameter 1 = lasso/SCAD, 0 =ridge controls the amount of penalty devided by sparceness and grouping effect producing penalties
    lambda <-Optimal4020uu[jj,1]
    AAAA <-  PEE_LI2(data=Sim100N40P20uu[[jj]],method='SCAD')
    
    EstimatesSCAD4020u[,jj] <- as.matrix(AAAA[[2]])
  }

save(EstimatesSCAD4020u, file="EstimatesSCAD4010u.Rda")
