...
source("Estimators and CV 6 10 fold cross.R")
load('Sim100N40P4u.Rda')
######
...
library(matlab)
  ...
  EstimatesSCAD4004 <- matrix(1,4,nsim)
  Optimal4004 <- matrix(1,nsim,2)
...
for(jj in 1:5){
    for (i_grid in 1:n_grid){
      ...
      resultaat <-  CV.PEE_LI(data=Sim100N40P4u[[jj]])
    ...
    Optimal4004[jj,] <- as.numeric(grid[LOW_MSE[1],])
    }
save(Optimal4004, file="Optimal4004.Rda")



  for(jj in 1:nsim){
    alpha <- Optimal4004[jj,2] #parameter 1 = lasso/SCAD, 0 =ridge controls the amount of penalty devided by sparceness and grouping effect producing penalties
    lambda <-Optimal4004[jj,1]
    AAAA <-  PEE_LI2(data=Sim100N40P4u[[jj]],method='SCAD')
    
    EstimatesSCAD4004[,jj] <- as.matrix(AAAA[[2]])
  }
save(EstimatesSCAD4004, file="EstimatesSCAD4004.Rda")
