...
library(matlab)
  ...
  EstimatesSCAD <- matrix(1,20,nsim)
...
for(jj in 1:50){
    for (i_grid in 1:n_grid){
      ...
      resultaat <-  CV.PEE_LI(data=Sim100N40P20uu[[jj]])
    ...
    Optimal4020uu1[jj,] <- as.numeric(grid[LOW_MSE[1],])
    }
save(Optimal4020uu1, file="Optimal4020uu1.Rda")
   
  for(jj in 1:nsim){
    alpha <- Optimal4020u[jj,2] #parameter 1 = lasso/SCAD, 0 =ridge controls the amount of penalty devided by sparceness and grouping effect producing penalties
    lambda <-Optimal4020u[jj,1]
    AAAA <-  PEE_LI2(data=Sim100N40P20u[[jj]],method='SCAD')
