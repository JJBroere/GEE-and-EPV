...
source("Estimators and CV 402.R")
load('Sim100N40P2u.Rda')
######
...
library(matlab)
  ...
  EstimatesSCAD4005 <- matrix(1,2,nsim)
...
for(jj in 1:nsim){
    for (i_grid in 1:n_grid){
      ...
      resultaat <-  CV.PEE_LI(data=Sim100N40P2u[[jj]])
    ...
    Optimal4002[jj,] <- as.numeric(grid[LOW_MSE[1],])
    }
save(Optimal4002, file="Optimal4002.Rda")



  for(jj in 1:nsim){
    alpha <- Optimal4002[jj,2] #parameter 1 = lasso/SCAD, 0 =ridge controls the amount of penalty devided by sparceness and grouping effect producing penalties
    lambda <-Optimal4002[jj,1]
    AAAA <-  PEE_LI2(data=Sim100N40P2u[[jj]],method='SCAD')
    
    EstimatesSCAD4002[,jj] <- as.matrix(AAAA[[2]])
  }
save(EstimatesSCAD4002, file="EstimatesSCAD4002.Rda")
