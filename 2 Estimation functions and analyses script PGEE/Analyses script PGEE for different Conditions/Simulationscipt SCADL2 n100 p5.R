...
load('Sim100N100P5u.Rda')#simulated data
...
EstimatesSCAD1005u <- matrix(1,5,nsim)
Optimal1005u <- matrix(1,nsim,2)
...
for(jj in 1:nsim){
  for (i_grid in 1:n_grid){
    ...
    resultaat <-  CV.PEE_LI(data=Sim100N100P5u[[jj]])
  ...
  Optimal1005u[jj,] <- as.numeric(grid[LOW_MSE[1],])
}
save(Optimal1005u, file="Optimal1005u.Rda")

for(jj in 1:nsim){
  alpha <- Optimal1005u[jj,2] #parameter 1 = lasso/SCAD, 0 =ridge controls the amount of penalty devided by sparceness and grouping effect producing penalties
  lambda <-Optimal1005u[jj,1]
  AAAA <-  PEE_LI2(data=Sim100N100P5u[[jj]],method='SCAD')
  
  EstimatesSCAD1005u[,jj] <- as.matrix(AAAA[[2]])
}

save(EstimatesSCAD1005u, file="EstimatesSCAD1005u.Rda")