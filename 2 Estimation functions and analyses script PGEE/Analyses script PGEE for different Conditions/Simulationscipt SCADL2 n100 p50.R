...
load('Sim100N100P50u.Rda')#simulated data
...
tresh_beta <- 0.00001 #for stability small enough, lots of coeff if a betaj closer than this treshold, set to zero 
...
EstimatesSCAD10050u <- matrix(1,50,nsim)
Optimal10050u <- matrix(1,nsim,2)
...
for(jj in 1:nsim){
  for (i_grid in 1:n_grid){
    ...
    resultaat <-  CV.PEE_LI(data=Sim100N100P50u[[jj]])
  ...
  Optimal10050u[jj,] <- as.numeric(grid[LOW_MSE[1],])
}
save(Optimal10050u, file="Optimal10050u.Rda")

tresh_beta <- 0.01
for(jj in 1:nsim){
  alpha <- Optimal10050uu[jj,2] #parameter 1 = lasso/SCAD, 0 =ridge controls the amount of penalty devided by sparceness and grouping effect producing penalties
  lambda <-Optimal10050uu[jj,1]
  AAAA <-  PEE_LI2(data=Sim100N100P50u[[jj]],method='SCAD')
  
  EstimatesSCAD10050u[,jj] <- as.matrix(AAAA[[2]])
}

save(EstimatesSCAD10050u, file="EstimatesSCAD10050u.Rda")
