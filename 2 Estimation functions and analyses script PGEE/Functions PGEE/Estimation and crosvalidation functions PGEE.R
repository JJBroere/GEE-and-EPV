...
CV.PEE_LI <- function(data)
{
  ...
  beta_cv <- array(dim=c(n_subj,p))
  conv_cv <- array(dim = c(n_subj))
  RSS_cv <- array(dim=c(n_subj))
  
  for (ident in 1:n_subj)
  {
    
    #Trow one subject out
    data_min_ident <- subset(data, id != ident)
    
    #id opnieuw nummeren 
    greater <- (data_min_ident$id > ident)
    data_min_ident$id[greater] <-  data_min_ident$id[greater] -1
    
    #Fit model
    result <- PEE_LI2(data_min_ident,method)  
